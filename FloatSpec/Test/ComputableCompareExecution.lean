...
`FaithfulPrimFloat.SFeqb`/`SFltb`/`SFleb`/`SFcompare` are `noncomputable`: their
finite case decides an equality between *real numbers*, which under `open
Classical` elaborates via `Classical.propDecidable`.  The `*C` variants in
`FloatSpec.IEEE754.ComputableCompare` decide the same relation by aligning
exponents over `Int`, and are proved equal to the specifications
(`SFltbC_eq`, `SFeqbC_eq`, `SFlebC_eq`, `SFcompareC_eq`).
