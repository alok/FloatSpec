...
`{coq_file}` roles, the `coq` quote block, and the Markdown citation linter. It checks the exact
hover text, the citation data stored in docstrings, and the errors for rejected citations.
...
without errors or warnings. Probing needs meta-level commands and a state rollback, which the
standalone fixture step forbids, so this is a module of the compiled test library: `lake build
FloatSpecTests` builds it with warnings as errors, and `scripts/check_compiled_trust.py --scope
tests` audits and kernel-replays it. `scripts/fixtures/CoqDocRole.lean` holds the accepted uses
under the fixture step's rules. Docstring text for rejected citations is built inside string
literals, so no line of this file opens a `coq` block that `scripts/validate_flocq_source_refs.py`
would try to check. -/

open Lean Elab Command

namespace FlocqCitations
...
run_cmd do
  let (messages, consts) ← FlocqCitations.probe (FlocqCitations.versoDoc "{coq}`Bcompare`")
...
run_cmd do
  expectMessages (versoDoc "{coq}`round_0`") #[
    "error: `round_0` resolves to the Lean declaration `FlocqCitations.Shadow.round_0`, which has no \
    `@[flocq_source]` anchor, but it is also the Coq name of\n  \
