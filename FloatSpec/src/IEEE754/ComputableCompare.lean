...
to an explicit local real-valued specification. Source-facing comparison remains
in `FaithfulPrimFloat`; no unconditional equivalence to that API is claimed. -/
