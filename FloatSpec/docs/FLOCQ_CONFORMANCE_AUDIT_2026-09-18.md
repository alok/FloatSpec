...
| `Core.FTZ.FTZ_format` | `src/Core/FTZ.v:36-41` requires an explicit finite float witness, normalized nonzero mantissa, and minimum exponent. `FTZ_format_generic` and `generic_format_FTZ` are separate source theorems at lines 106 and 80. | Lean defined `FTZ_format` to be `generic_format beta FTZ_exp`, so both conversion theorems were definitional restatements and the source carrier was absent. | Replaced the definition with the source-shaped existential; conversion theorems are direct implications. `FTZ_format_iff_generic` is now **proved** in both directions (`FTZ_generic_format_run` via `generic_inclusion_mag` from FLXN, and `FTZ_format_generic_run` via `mag_generic_gt` ruling out the flush branch for nonzero inputs) and supports downstream generic-grid callers. Independent zero-witness and direct `FLXN_format_FTZ` regressions check the structural carrier. |
...
seven Lean-only theorem payloads or local proof adapters are classified.
